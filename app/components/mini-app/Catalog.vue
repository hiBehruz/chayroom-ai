...
const { data: guides } = useAsyncData<unknown[]>('catalog-guides', () => $fetch('/api/guides'))
const { data: courses } = useAsyncData<unknown[]>('catalog-courses', () => $fetch('/api/courses'))
...
<template>
  ...
  >
    ...
    >
      <div
        class="font-sans"
        style="font-size:28px;font-weight:800;letter-spacing:-0.03em;line-height:1.1;color:#0f1115;margin:0"
      >
        Materiallar
      </div>
      <div
        class="font-sans"
        style="font-size:14px;margin:4px 0 0;color:#6b7280;font-weight:400"
      >
        O'rganish formatini tanlang.
      </div>
    </div>
    ...
    >
      ...
      >
        <div style="width:44px;height:44px;border-radius:50%;background:rgba(245,158,11,0.12);display:grid;place-items:center;flex-shrink:0">
          <UIcon
            name="solar:notes-minimalistic-bold"
            class="size-5.5"
            style="color:#f59e0b"
          />
        </div>
      ...
      >
        <span
          ...
          style="position:absolute;top:14px;right:14px;font-size:9.5px;font-weight:700;padding:4px 8px;border-radius:999px;letter-spacing:.06em"
        >Tez kunda</span>
        <div :style="`width:44px;height:44px;border-radius:50%;background:${soon.iconBg};display:grid;place-items:center`">
          <UIcon
            :name="soon.icon"
            class="size-5.5"
            :style="`color:${soon.color}`"
          />
        </div>
        <div>
          <h3 style="font-family:inherit;font-weight:800;font-size:13px;font-weight:600;letter-spacing:-0.01em;margin:0;color:#1a1a1a">
            {{ soon.name }}
          </h3>
        </div>
      </div>
    ...
    <div style="padding:0 20px 20px">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <div style="font-size:11px;color:#6b7280;font-weight:500;margin-top:3px">
            Mavzu taklif qiling — biz qo'shamiz
          </div>
        </div>
        <svg
          width="14"
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <div
            class="w-full"
            style="background:#fff;border-radius:24px 24px 0 0;padding:20px 20px 36px;max-width:390px;border-top:1px solid #e5e7eb"
          >
            <div style="width:36px;height:4px;border-radius:999px;background:#e5e7eb;margin:0 auto 20px" />

            <div class="flex items-start justify-between mb-2">
              <h2 style="font-size:18px;font-weight:800;letter-spacing:-0.02em;color:#0f1115;margin:0">
                Material so'rash
              </h2>
              <button
            ...
            <p style="font-size:13px;line-height:1.6;color:#6b7280;margin:0 0 18px">
              Qaysi mavzuni ko'rishni istayotganingizni ayting. Kontent rejalashtirish jarayonida hisobga olamiz.
            </p>

            <div class="mb-3">
              <label
                class="block mb-2"
                style="font-size:10px;font-weight:800;letter-spacing:0.08em;text-transform:uppercase;color:#9aa0a8"
              >MATERIAL NIMA HAQIDA?</label>
              <input
                v-model="topic"
                type="text"
            ...
            <div class="mb-5">
              ...
              <div class="relative">
                ...
                <span
                  ...
                  style="font-size:10px;color:#c4c8d2"
                >{{ details.length }}/500</span>
              </div>
            </div>
