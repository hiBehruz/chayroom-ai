...
<template>
  <div
    v-if="course"
    class="min-h-screen"
    :class="isMiniApp ? 'bg-[#efefef] is-mini' : 'bg-cx-surface'"
  >
    <!-- Preview image wrapper -->
    <div
      class="mx-auto"
      :class="isMiniApp ? 'px-4 pt-5 max-w-full' : 'w-310 max-w-[calc(100vw-40px)] px-0 pt-8 max-md:px-4'"
    >
      <div
      ...
      >
        ...
        <div
          class="rounded-2xl overflow-hidden mb-6 relative max-md:!h-[180px]"
          :style="{ height: isMiniApp ? '180px' : '400px', backgroundColor: course.bg }"
        >
          ...
          >
            <div class="course-preview-copy flex items-center gap-6">
              <div>
                <p class="text-[12px] font-bold text-cx-muted mb-2 uppercase tracking-wide">
                  Kurs:
                </p>
              ...
              <div class="flex flex-col gap-3 items-end opacity-50">
                <UIcon
                  ...
                  class="size-10 text-cx-blue"
                />
                <UIcon
                  name="i-lucide-bot"
                  class="size-14 text-[#1a1a1a]"
                />
                <UIcon
                  name="i-lucide-pen-line"
                  class="size-8 text-cx-blue"
                />
              </div>
            </div>
          </div>
          ...
          <div
            v-else
            class="course-preview-copy absolute inset-0 flex items-center justify-center gap-5"
          >
            <UIcon
              name="i-lucide-globe"
              class="size-16 text-[#f97316] shrink-0"
            />
            <div>
              <p class="course-preview-title text-[28px] font-extrabold text-white leading-tight mb-2">
                Vibe coding noldan
              </p>
              <p class="text-[14px] text-white/60 leading-normal">
                Kod bilmasdan AI yordamida saytlar, vositalar va ilovalar yaratish
              </p>
            </div>
          </div>
        </div>
    ...
    <div
      class="mx-auto pb-8"
      :class="isMiniApp ? 'px-4 max-w-full' : 'w-310 max-w-[calc(100vw-40px)] px-0 max-md:px-4'"
    >
      <div
        class="max-w-full"
        :class="isMiniApp ? '' : 'mx-auto w-220.5'"
      >
        <!-- Course info -->
        <div class="mb-6">
          <h1
            class="course-detail-title font-extrabold tracking-tight text-[#1a1a1a] leading-tight mb-2"
            :class="isMiniApp ? 'text-[26px]' : 'text-[48px]'"
          >
            {{ course.title }}
          </h1>
          <p
            class="course-detail-desc text-cx-muted leading-[1.6] mb-3"
            :class="isMiniApp ? 'text-[15px]' : 'text-[20px]'"
          >
            {{ course.desc }}
          </p>
          <div
            class="course-detail-meta flex items-center gap-5 text-cx-muted mb-4"
            :class="isMiniApp ? 'text-[13px]' : 'text-[20px]'"
          >
        ...
        <div
          class="course-detail-grid grid items-start"
          :class="isMiniApp ? 'grid-cols-1 gap-5' : 'grid-cols-[1fr_300px] gap-10 max-md:grid-cols-1'"
        >
          <!-- LEFT: modules -->
          <div>
            <!-- Rich content -->
            ...
            <h2
              class="course-detail-section-title flex items-center gap-3 font-extrabold text-[#1a1a1a] mb-4"
              :class="isMiniApp ? 'text-[18px]' : 'text-[40px]'"
            >
              <UIcon
                name="i-solar-bill-list-bold"
                :class="isMiniApp ? 'size-5' : 'size-9'"
              />
              Kurs tarkibi
            </h2>
            ...
            >
              ...
              >
                ...
                >
                  <div
                    class="flex size-7 items-center justify-center rounded-full shrink-0 text-[12px] font-bold"
                    :style="openModules.has(modIndex) ? 'background:#3480f1;color:#fff' : 'background:#efefef;color:#70707a'"
                  >
                    {{ modIndex + 1 }}
                  </div>
                ...
                <div
                  v-if="openModules.has(modIndex)"
                  class="flex flex-col gap-px px-3 pb-3"
                >
                  <NuxtLink
            ...
            <div class="rounded-2xl border border-cx-line bg-[#f7f5ef] overflow-hidden">
              <div class="flex items-center justify-between px-5 py-4 border-b border-cx-line">
                <div class="flex items-center gap-3">
                  <UIcon
                    name="i-solar-layers-minimalistic-bold"
                    class="size-5 text-cx-blue shrink-0"
                  />
                  <span class="text-[15px] text-cx-muted font-medium">Modullar</span>
                </div>
                <span
                  class="font-bold text-[#1a1a1a] tabular-nums"
                  :class="isMiniApp ? 'text-[15px]' : 'text-[20px]'"
                >{{ course.modules }}</span>
              </div>
              <div class="flex items-center justify-between px-5 py-4 border-b border-cx-line">
            ...
            <div class="rounded-2xl border border-cx-line bg-[#f7f5ef] overflow-hidden">
                ...
                <ul class="flex flex-col gap-3">
                  ...
                  >
                    <UIcon
                      ...
                      class="size-4.5 text-emerald-500 shrink-0"
                    />
                    {{ item }}
                  </li>
                </ul>
              ...
              <div class="border-t border-cx-line px-5 py-4">
                ...
                >
                  <UIcon
                    name="i-solar-play-circle-bold"
                    class="start-btn-icon size-5 shrink-0"
                  />
