...
test('guide editor exposes premium content blocks', async () => {
  ...
  assert.match(editor, /content:\s*'inline\*'/)
...
test('guide detail copy blocks write their content to the clipboard', async () => {
  ...
  assert.match(css, /\.rich-content \[data-copy-pre\],[\s\S]*\.guide-editor-body \[data-copy-pre\] \{[\s\S]*cursor:\s*text/)
  assert.match(css, /\.rich-content \[data-copy-pre\],[\s\S]*\.guide-editor-body \[data-copy-pre\] \{[\s\S]*user-select:\s*text/)
